  ...
  reducers: {
    disponibilidade: {
      [BUSCAR_DISPONIBILIDADE]: (
        ...
        (pgDisponibilidade || []).forEach(
          ({dia_semana, disponibilidade_id}) => {
            diasSemanaCopy[dia_semana].data = diasSemanaCopy[
              dia_semana
            ].data.map(v => {
              const {disponibilidade_id: disponibilidade_id2} = v;
              const data =
                disponibilidade_id === disponibilidade_id2 ? date : v.data;
              return {...v, data};
            });
          },
        );
      ...
      [TOOGLE_SELECT_DISPONIBILIDADE]: (state, {index}) => {
        ...
        const data = (state.semana[diaSelecionado].data[index].data = empty(
          dataTmp,
        )
          ? moment().format()
          : null);
