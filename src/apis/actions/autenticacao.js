...
const checarSeAppEstaAtualizado = versao_app => {
  if (!empty(versao_app)) {
    ...
    const currentVersion = Number(
      `${v[0]}${pad(3, v[1], '0')}${pad(4, v[2], '0')}`,
    );
    if (currentVersion < versao_app) {
      Alert.alert(
        'ATUALIZAÇÃO DISPONÍVEL',
        'Nova vesão do JogoRápido disponível. Manter seu aplicativo atualizado garante uma boa experiência de navegação. Deseja atualizar?',
        [
          {text: 'Agora não', style: 'cancel'},
          {
            ...
            onPress: () => {
              if (Platform.OS === 'ios') {
                Linking.openURL(
                  `https://itunes.apple.com/us/app/${applestoryid}`,
                );
              } else {
                Linking.openURL(
...
export const actionAutenticar = (hasDispatchRedux = true) => {
  return new Promise((_resolve, _reject) => {
    ...
    AsyncStorage.getItem(AUTENTICAR)
      .then(value => {
        if (value != null) {
        ...
        if (_r.isCancelled) {
          ...
          AccessToken.getCurrentAccessToken().then(data => {
            ...
            if (empty(accessToken)) {
              _reject({
                status: 'erro',
                mensagem: 'Falha ao obter dados de seu login no facebook',
              });
            } else {
              fetch(
...
export const actionCriarContaViaFormulario = actionFetchItem(
  CRIAR_CONTA,
  'usuario/criar-conta',
);
export const actionLogin = actionFetchItem(CONECTAR, 'usuario/login');
export const actionRecuperarSenha = actionFetchItem(
  RECUPERAR_SENHA,
  'usuario/recuperar-senha',
);
export const actionChecarTokenEmail = actionFetchItem(
  CHECAR_SIMBOLO_EMAIL,
  'usuario/checar-token-email',
);
export const actionAlterarSenha = actionFetchItem(
  ALTERAR_SENHA,
  'usuario/alterar-senha',
);
export const actionValidarEmail = actionFetchItem(
  VALIDAR_EMAIL,
