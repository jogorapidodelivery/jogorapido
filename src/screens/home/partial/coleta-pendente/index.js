...
export default class ColetaPendente extends PureComponent {
  _abrirPedido = ({acao}) => {
    if (acao !== 'cancelar') {
      ...
      coletaAtualizarStatus({body_rsa})
        .then(() => {
  ...
  _submit = () => {
    this.props.navigation.push('confirma', {
      params: {
        ...
        mensagem:
          'Ao clicar em “Confirmar” declaro que estou ciente que devo cumprir as normas que constam no Regulamento do <strong>APP JogoRápido.</strong>',
        button: {
          text: {
    ...
    return (
      <View style={styl.container}>
        <AnimatableText
          ...
          style={stylDefault.h1}>
          {tituloColetas}
        </AnimatableText>
        <ViewAnimatable animation="fadeInUp" useNativeDriver={true} delay={300}>
          {coleta.map((v, k) => (
            <ItemRoute {...v} key={`item-${k}`} total={total} index={k} />
          ))}
        </ViewAnimatable>
        <ViewAnimatable
          useNativeDriver={true}
          ...
          <Button
            text={{
              value: (
                <Text>
                  Aceitar <Text style={stylDefault.normal}>corrida</Text>
                </Text>
              ),
              color: '07',
            }}
