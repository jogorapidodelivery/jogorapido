...
const infos = [
  'Vamos começar? Seus dados ficarão\n salvos em um ambiente seguro\nda ',
  'JogoRápido',
];
...
export default class CadastrarPlayer extends PureComponent {
  ...
  _onCreateAccount = _callBackUnlock => {
    this.form
      ...
      .then(response => {
        actionCriarContaViaFormulario(response)
          .then(() => {
            _callBackUnlock();
    ...
    return (
      <BaseScreen
        ...
        <AnimatableView animation="fadeInUp" useNativeDriver={true} delay={800}>
          <Button
            text={{value: 'CADASTRAR', color: '07'}}
            onPressAwait={this._onCreateAccount}
            bg="14"
            style={styl.button}
          />
        </AnimatableView>
