...
export const openPageStart = async (navigation, delay = 0) => {
  ...
  if (getItemByKeys(state, 'autenticacao.usuario_id') !== undefined) {
    let {
      autenticacao: {email, usuario, usuario_id, email_verificado, coleta},
    } = state;
    if (empty(usuario)) {
      usuario = email;
    }
    if (usuario_id) {
      await setUserBackground({usuario_id});
    }
    if (email_verificado) {
...
export const findRenderPage = coleta => {
  ...
  if (!empty(coleta) && coleta.length > 0) {
    ...
    if (
      [2, 3, 4, 5, 6].indexOf(status_coleta_id) !== -1 &&
      empty(data_checkout_cliente)
    ) {
      _router = 'coletar';
    } else {
      _router = 'home';
