...
let aguia = new Sound(
  'http://177.101.149.36/aguia_small.mp3',
  Sound.MAIN_BUNDLE,
  error => {
    if (error) {
      aguia = undefined;
...
const prepareParamsDispatchNotifier = response => {
  let {
    ...
    tempo_aceite,
    type,
    coleta: [{data_hora_atual, data_notificacao, status_coleta_id}],
  } = response;
  let coleta_id = '';
  if (response.coleta.length > 0) {
    coleta_id = response.coleta.map(v => {
      return v.coleta_id;
    });
    if (coleta_id.length > 1) {
      let coleta_id_lasted = coleta_id.pop();
      coleta_id = `As coletas ${coleta_id.join(
        ', ',
      )} e ${coleta_id_lasted} está aguardando sua decisão`;
    } else {
      coleta_id = `A coleta ${coleta_id.join(
        ', ',
  ...
  if (tempo_aceite) {
    const mill =
      moment(data_hora_atual, 'AAAA-MM-DD H:mm:ss').diff(
        moment(data_notificacao, 'AAAA-MM-DD H:mm:ss'),
      ) / 1000;
    const tempo_aceite_restante = tempo_aceite - mill;
    if (
      tempo_aceite_restante > 0 &&
      tempo_aceite_restante <= tempo_aceite &&
      Number(status_coleta_id) === 1
    ) {
      return {
        response,
        tempo_aceite,
        tempo_aceite_restante,
        acao,
        type,
...
const renderNotifierDisplay = (
  notification,
  tempo_aceite,
  _tempo_aceite_restante,
  coleta_id,
  _resolve,
  _reject,
) => {
  console.log({
    action: 'dispatchNotify/renderNotifierDisplay',
    tempo_aceite,
    coleta_id,
  });
  if (empty(coleta_id)) {
  ...
  const loopInterval = () => {
    ...
    if (counter > tempo_aceite) {
      if (timerInProcess !== undefined) {
        clearInterval(timerInProcess);
      }
...
export const triggerNotifier = message =>
    ...
    if (!empty(GrupoRotas.store)) {
      ...
      if (!empty(state)) {
        if (!empty(state.autenticacao)) {
          let {
            autenticacao: {tempo_aceite, usuario_id, entregador_id},
          } = state;
          if (!empty(message)) {
            let blnDispatchNotifier;
            const {acao, type} = message;
            if (
              'criar-coleta, editar-coleta, apagar-coleta, nova-coleta'.indexOf(
                acao,
              ) !== -1
            ) {
              try {
                ...
                if (
                  ...
                  if (coleta.length > 0) {
                    ...
                    blnDispatchNotifier = prepareParamsDispatchNotifier(
                      message,
                    );
                    if (blnDispatchNotifier) {
                      ...
                      return renderNotifierDisplay(
                        ...
                        () => {
                          ...
                          GrupoRotas.store.dispatch({
                            type: COLETA_NOVA_TEMPO_EXPIRADO,
                          });
                          triggerDestroyTimerProgress();
              ...
              console.log('acao ERR', acao);
            }
          } else {
            console.log('message EMPTY');
          }
        } else {
          console.log('autenticacao EMPTY');
        }
      } else {
