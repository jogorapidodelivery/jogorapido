...
export const upperCamelCase = v => {
  ...
  v = v.replace(/[a-zA-Z\u00C0-\u00FF]+/g, function(w) {
    ...
    if (
      (_end.length > 2 || _end.length === 0) &&
      _rejectUperCase.indexOf(_end) === -1
    ) {
...
export const embaralharEmail = v => {
  ...
  return (
    _parts[0].substring(0, 2) +
    '*'.repeat(_parts[1].length - 3) +
    '@' +
    _parts[1]
  );
};
...
export const captalize = (v, force = true) => {
  ...
  return (
    v.charAt(0).toUpperCase() + (force ? v.slice(1).toLowerCase() : v.slice(1))
  );
};
...
export const abreviarNome = (s, minCharAbreviar = 10) => {
  ...
  if (s.length > minCharAbreviar) {
    ...
    if (_nome.length > 1) {
      ...
      _nome[t] =
        _nome[t].substring(0, 1).toUpperCase() +
        _nome[t].substring(1, _nome[t].length).toLowerCase();
    }
    return _nome.join(' ');
  } else {
