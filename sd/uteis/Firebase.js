...
export const initFirebase = (_callBack = undefined) => {
  ...
  const _sucessToken = _token => {
    ...
    firebase
      .notifications()
      .onNotificationOpened(async ({notification: {_data: data}}) => {
        console.log('ON_OPENED', new Date());
        try {
          return await _dataNotification({
            ...data,
            type: 'DISPLAY',
            listener: 'ON_OPENED',
          });
    ...
    firebase.messaging().onMessage(async ({_data: data}) => {
      console.log('ON_DISPLAY_MESSAGING', new Date(), data);
      try {
        return await _dataNotification({
          ...data,
          type: 'DISPLAY',
          listener: 'ON_DISPLAY_MESSAGING',
        });
      } catch (_err) {
    ...
    try {
      if (firebase.messaging().setBackgroundMessageHandler !== undefined) {
        firebase
          .messaging()
          .setBackgroundMessageHandler(async ({_data: data}) => {
            console.log('ON_BACKGROUND_MESSAGING', new Date());
            try {
              return await _dataNotification({
                ...data,
                type: 'DISPLAY',
                listener: 'ON_BACKGROUND_MESSAGING',
              });
            } catch (_err) {
              return Promise.resolve();
            }
      ...
      .catch(_e => {
        setTimeout(() => {
          Alert.alert(
            ...
            'Ative o recurso de receber mensagens via push para se manter atualizado. Deseja ativar o recebimento de push?',
            [
