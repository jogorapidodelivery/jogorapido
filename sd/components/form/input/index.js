...
import { TextInput, View, Text, Keyboard, TouchableOpacity, Vibration } from "react-native";
...
export default class Input extends Component {
    ...
    constructor(props) {
        ...
        this._bgValidate = value.length === 0 ? this._dc : (this._isValid ? this._sc : this._wc);
    ...
    _formatValue(_str = undefined) {
        if (_str === undefined) _str = this.props.value
        const { type } = this.props
        if (type === "data") return InputDate.tratarDados({ value: _str, ...this.props }).valueRender
    ...
    _setValue = (value) => {
        const {type} = this.props
        if (type === "data") this._modal && this._modal.current && this._modal.current.setValue(value)
    ...
    _renderIcon(_action) {
        ...
        return value ? <TouchableOpacity onPress={onPress} style={stylWarpIcon}>
    ...
    get _renderItem() {
        const { disabled, returnKeyType, onSubmit, placeHouder, type, max} = this.props
        const { stylWarpInput, stylTextInput, stylLavel, stylWarpWarning, stylBlobWarning } = _styl[this._styleName];
        const { isFocus, value, stylErrorAnimatable} = this.state
        const backgroundColor = this._bgValidate
        let _attr = _attributes(type || "");
        if (max !== undefined) _attr.maxLength = max;
        return <Fragment>
            {this._renderIcon("leftIcon")}
            {(isFocus || value.length > 0) && <View style={stylWarpWarning}>
                <View style={[stylBlobWarning, { backgroundColor }]} />
            </View>}
            <View style={stylWarpInput}>
                {(isFocus || value.length > 0) && placeHouder !== "" && <Text style={stylLavel}>{placeHouder}</Text>}
                <TextInput
                    ref={this._input}
                    {..._attr}
                    placeholderTextColor={stylErrorAnimatable ? stylErrorAnimatable : this._dc}
                    style={[stylTextInput, (stylErrorAnimatable && { color: stylErrorAnimatable || "gray"})]}
                    editable={!disabled}
    ...
    get _renderTouchable() {
        ...
        return <Fragment>
            ...
            <TouchableOpacity style={[stylContainer, style]} onPress={this._pressModal.bind(this, true)}>
                {this._renderItem}
            </TouchableOpacity>
        </Fragment>
    }
    get _renderDef() {
        ...
        const { stylContainer, stylInderline, stylWarpConteiner } = _styl[this._styleName];
        return <View style={[stylContainer, style]}>
            <View style={stylWarpConteiner}>
                {this._renderItem}
            </View>
            {underline && <View style={[stylInderline, { backgroundColor: stylErrorAnimatable ? stylErrorAnimatable : cor[underline.color]}]}></View>}
        </View>
    }
    focus = () => {
    ...
    render() {
        const { type } = this.props
        return <ViewAnimatable useNativeDriver={true} ref={_ref => this._ViewAnimatable = _ref}>
