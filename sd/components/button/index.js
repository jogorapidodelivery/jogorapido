...
export default class Button extends PureComponent {
  ...
  constructor(props) {
    ...
    if (bg === undefined) {
      this._action = this._renderOpacity;
    } else {
      this._action = this._renderHighlight;
    }
  }
  get _text() {
    ...
    return value ? (
      <Text style={[text, spaceIcon, {color: cor[color]}, style]}>{value}</Text>
    ) : (
      <Fragment />
    );
  }
  get _textCounter() {
    ...
    return value ? (
      <View style={[warpCounter, {backgroundColor: cor[bg || '12']}]}>
        <Text style={[textCounter, {color: cor[color]}]}>{value}</Text>
      </View>
    ) : (
      <Fragment />
    );
  ...
  _icon(_action) {
    ...
    const warpPos =
      this.props.text && this._styleName === 'padrao'
        ? {
            position: 'absolute',
            top: 0,
    ...
    return (
      value && (
        <View style={[warpIcon, warpPos]}>
          <Text style={[textIcon, {color: cor[color || '01']}, style]}>
            {value}
          </Text>
        </View>
      )
  ...
  _renderWarp = _href => {
    ...
    if (typeof _cor === 'string') {
      return (
        <View style={[warp, {backgroundColor: _cor}, style]}>{_href}</View>
      );
    }
    let _props = {};
    if (gradientLocations) {
      _props.locations = gradientLocations;
    }
    return (
  ...
  _renderHighlight = () => {
    ...
    return this._renderWarp(
      <TouchableHighlight
        activeOpacity={disable ? 0.5 : 1}
        onPress={this.onPress}
        underlayColor={this._highlightOver}
        style={btn}>
        <Fragment>
          {this._icon('leftIcon')}
          {this._text}
